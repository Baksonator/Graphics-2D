...
public class Transition extends GameState
{
	...
	public void render(Graphics2D g, int sw, int sh)
	{
		...
		case LeftRightSquash:
			...
			if (position < 0.5) {
				Shape triangle = calcTriangle(0, host.getHeight() / 2, 0, 0,
						(int)(2 * position * host.getWidth()), 0);
				g.setClip(triangle);
				g.drawImage(endImage, 0, 0, null);
				
				Shape triangle2 = calcTriangle(host.getWidth(), host.getHeight() / 2, host.getWidth(),
						host.getHeight(), host.getWidth() - (int)(2 * position * host.getWidth()),
						host.getHeight());
				g.setClip(triangle2);
				g.drawImage(endImage, 0, 0, null);
			} else {
				Shape trapez = calcTrapez(0, host.getHeight() / 2, 0, 0, host.getWidth(), 0,
						host.getWidth(), (int)(2 * (position - 0.5) * (host.getHeight() / 2)));
				g.setClip(trapez);
				g.drawImage(endImage, 0, 0, null);
				
				Shape trapez2 = calcTrapez(host.getWidth(), host.getHeight() / 2, host.getWidth(),
						host.getHeight(), 0, host.getHeight(), 0, host.getHeight() - 
