...
public class Transition extends GameState {
	...
	public static enum TransitionType {
		Crossfade, ZoomIn, ZoomOut, SwipeLeft, SwipeRight, LeftRightSquash
	};

	// Trenutno odabrana vrsta tranzicije
	private TransitionType type = TransitionType.SwipeLeft;
	...
	public Transition(GameHost host) {
		...
		// Slike unaprijed konstruisemo, jer ce uvijek biti istih dimenzija (prozor se
		// ne moze
	...
	public void render(Graphics2D g, int sw, int sh) {
		...
		// promjenjive
		// position, tj. ovdje na licu mjesta izracunavamo sve pozicije i velicine
		// oslanjajuci
		// se samo na tu promjenjivu. Eventualne dodatne tranzicije bi bilo dobro
		// implementirati
		...
		case LeftRightSquash:
			g.drawImage(startImage, 0, 0, null);

			if (position < 0.5) {
				Shape triangle = calcTriangle(0, host.getHeight() / 2, 0, 0, (int) (2 * position * host.getWidth()), 0);
				g.setClip(triangle);
				g.drawImage(endImage, 0, 0, null);

				Shape triangle2 = calcTriangle(host.getWidth(), host.getHeight() / 2, host.getWidth(), host.getHeight(),
						host.getWidth() - (int) (2 * position * host.getWidth()), host.getHeight());
				g.setClip(triangle2);
				g.drawImage(endImage, 0, 0, null);
			} else {
				Shape trapez = calcTrapez(0, host.getHeight() / 2, 0, 0, host.getWidth(), 0, host.getWidth(),
						(int) (2 * (position - 0.5) * (host.getHeight() / 2)));
				g.setClip(trapez);
				g.drawImage(endImage, 0, 0, null);

				Shape trapez2 = calcTrapez(host.getWidth(), host.getHeight() / 2, host.getWidth(), host.getHeight(), 0,
						host.getHeight(), 0, host.getHeight() - (int) (2 * (position - 0.5) * (host.getHeight() / 2)));
				g.setClip(trapez2);
				g.drawImage(endImage, 0, 0, null);
			}
			break;
	...
	private Shape calcTriangle(int x1, int x2, int y1, int y2, int z1, int z2) {
	...
	private Shape calcTrapez(int x1, int x2, int y1, int y2, int z1, int z2, int t1, int t2) {
		GeneralPath path = new GeneralPath();
		path.moveTo(x1, x2);
		path.lineTo(y1, y2);
		path.lineTo(z1, z2);
